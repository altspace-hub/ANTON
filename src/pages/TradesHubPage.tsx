    ...
    return (
      <div>
        <label className="mb-1 flex items-center gap-1.5 text-xs font-medium text-adv-gray">
          {label}
          {wasExtracted && <span className="rounded-full bg-adv-gold/10 px-1.5 py-0.5 text-[9px] text-adv-gold">from your doc</span>}
        </label>
        <input
          type={type}
          ...
          className="w-full rounded-lg border border-border bg-adv-dark px-3 py-2 text-sm text-adv-off-white placeholder:text-adv-gray-med focus:border-adv-gold focus:outline-none focus:ring-1 focus:ring-adv-gold"
        />
      </div>
    );
  }
      ...
      <div>
        ...
        {uploadMode === 'paste' ? (
          <textarea
            ...
            rows={8}
            className="w-full resize-none rounded-xl border border-border bg-adv-dark px-4 py-3 text-sm text-adv-off-white placeholder:text-adv-gray-med focus:border-adv-gold focus:outline-none focus:ring-1 focus:ring-adv-gold"
          />
        ) : (
          <label className={`flex cursor-pointer flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed px-6 py-10 text-center transition-colors ${
            ...
            {selectedFile ? (
              ...
              <>
                ...
                <p className="text-xs text-adv-gray-med">PDF · DOCX · TXT · MD — up to 20 MB</p>
              </>
            )}
          </label>
        )}
    ...
    return (
      <div>
        ...
        <div className="mb-3 flex items-center gap-2">
          ...
          {found.length > 0 && (
            <span className="text-xs text-adv-gray-med">· {found.length} fields found</span>
          )}
        </div>
              ...
              return (
                <div key={k} className="flex items-start gap-2 rounded-lg bg-adv-dark px-3 py-2">
                  ...
                  <div className="min-w-0">
                    <p className="text-[10px] text-adv-gray-med">{FIELD_LABELS[k] ?? k}</p>
    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        ...
        {extraction && (
          <button
            onClick={() => setPhase('upload')}
            className="flex items-center gap-1 text-xs text-adv-gray-med hover:text-adv-teal transition-colors"
          >
      ...
      {extraction && (extraction.foundFields?.length ?? 0) > 0 && (
        <div className="mb-4 rounded-lg border border-adv-gold/20 bg-adv-gold/5 px-3 py-2">
          <p className="text-xs text-adv-gold">
            Fields marked <span className="inline-block rounded-full bg-adv-gold/15 px-1.5 py-0.5 text-[9px]">from your doc</span> were filled automatically — review them before saving.
          </p>
        </div>
      )}
        ...
        <div>
          <label className="mb-1 flex items-center gap-1.5 text-xs font-medium text-adv-gray">
            ...
            {extraction?.foundFields?.includes('vatRegistered') && (
              <span className="rounded-full bg-adv-gold/10 px-1.5 py-0.5 text-[9px] text-adv-gold">from your doc</span>
            )}
          </label>
          <select
            value={identity.vatRegistered ? 'yes' : 'no'}
            onChange={e => setIdentity(prev => ({ ...prev, vatRegistered: e.target.value === 'yes' }))}
            className="w-full rounded-lg border border-border bg-adv-dark px-3 py-2 text-sm text-adv-off-white focus:border-adv-gold focus:outline-none"
          >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2">
          <label className="mb-1 block text-xs font-medium text-adv-gray">
            Standard notes or disclaimers to always include <span className="text-adv-gray-med">(optional)</span>
          </label>
          <textarea
            ...
            rows={3}
            className="w-full resize-none rounded-lg border border-border bg-adv-dark px-3 py-2 text-sm text-adv-off-white placeholder:text-adv-gray-med focus:border-adv-gold focus:outline-none focus:ring-1 focus:ring-adv-gold"
          />
        </div>
      </div>
