 ...
 *
 * Two modes:
 *   - 'create' — first use on a device with no usable biometric: the user
 *     picks a PIN and confirms it. Resolves with the chosen PIN; the host
 *     stores it (services/payment-pin.ts) and treats the payment as authorized.
 *   - 'enter'  — a PIN already exists: the user types it to authorize. The
 *     host VERIFIES it; a wrong attempt is signalled back by bumping
 *     `attemptFailures`, which clears the field and starts the back-off — the
 *     same 5-attempt model as PassphrasePromptModal.
 *
...
interface Props {
  /** 'create' shows a confirm field; 'enter' is a single PIN entry. */
  mode: 'create' | 'enter';
  /** Optional context line under the title. */
  reason?: string;
  /** Called with the entered PIN (create: the confirmed PIN). */
  onSubmit: (pin: string) => void;
  /** Called when the user cancels / closes. */
  onCancel: () => void;
  /** Wrong-PIN count in 'enter' mode; the host bumps it per failed verify. */
...
export default function PinPromptModal({
  ...
  // A wrong attempt (host bumped attemptFailures) clears + backs off.
  useEffect(() => {
