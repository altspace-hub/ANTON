/**
 * payment-pin.ts — in-app numeric PIN that authorizes a payment when no
 * strong biometric is available on the device.
 *
 * Why this exists: the device biometric gate (services/biometric.ts) requires
 * an enrolled fingerprint/face. Many phones either have no usable biometric
 * hardware or no enrolled biometric, and the @capgo plugin cannot fall back
 * to the OS PIN/pattern on Android. So executePayment uses this app-level PIN
 * as the fallback factor: a user-presence check that works on any device.
 *
 * Security model (mirrors wallet-passphrase.ts at a smaller scale):
 *   - The PIN itself is NEVER stored. We store a PBKDF2-HMAC-SHA256 hash with
 *     a per-PIN random salt. Verification re-derives and compares in constant
 *     time.
 *   - The {salt, hash} envelope goes through secure-store, which on a real
 *     device is wrapped again by the OS Keystore (AES-GCM). So the at-rest
 *     form is Keystore(PBKDF2(pin)). On web/dev secure-store falls back to a
 *     plain store — fine for the dev path.
 *   - A 4–8 digit PIN is low-entropy, so this is a user-presence / casual-
 *     access gate, not a defence against an attacker who already has the
 *     unlocked phone AND can read Keystore. That matches the biometric gate's
 *     own threat model (fresh presence check before signing).
 */
