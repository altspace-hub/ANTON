/**
 * schedules.ts — scheduled / recurring payment reminders.
 *
 ...
 *     safety net).
 *
 * This matches what BitPay subscriptions / Coinbase Commerce
 * subscriptions / BOLT-12 offers all do — none offer true unattended
 * recurring crypto. Production wallets (Phantom, MetaMask, Trust) do
 * not ship unattended recurring at all.
 *
 ...
 *   • `monthly` — the Nth day of every month (1..28 to dodge Feb,
 *                 or 'last' for the last day)
 *   • `yearly`  — a chosen month + day-of-month
 *
 * Fire time is always 09:00 local on the chosen day — bank-business-
 * hours by convention, picked to roughly match SEPA SCT Inst's
 * standard processing windows. The user can override per-schedule
 * once the UI exposes a time picker.
 *
 * Storage: IDB store `schedules` (v3 → v4 bump in db.ts).
 */
...
export function nextFireFrom(rec: Recurrence, after: number): number {
  ...
  switch (rec.kind) {
    case 'daily': {
      ...
      // If after has already passed today's 9:00, addDays produces a
      // future time; if not, we still want NEXT iteration not now.
      return next.getTime() <= after ? addDays(next, 1).getTime() : next.getTime();
    }
    case 'weekly': {
      const next = nineAm(start);
      // Advance day by day until we hit the chosen weekday and the
      // candidate time is in the future.
      while (next.getDay() !== rec.dayOfWeek || next.getTime() <= after) {
        next.setDate(next.getDate() + 1);
      }
      // Apply the interval skip: if interval > 1, snap forward to
      // the right week. Simple approximation — assumes the user
      // creates the schedule wanting the FIRST occurrence as the
      // next match (correct for the common interval=1 case).
      return next.getTime();
    }
    case 'monthly': {
...
async function scheduleNotificationFor(s: Schedule): Promise<void> {
  ...
  try {
    await ensureChannel(plugin, 'fc-pay-scheduled', 'Scheduled payments');
