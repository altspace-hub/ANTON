/**
 * behavior-profile.ts — derive the customer's empirical money pattern
 * from their local payment history.
 *
 * Pure logic, no I/O — `deriveBehaviorProfile` is the testable core.
 * The profile is NOT stored: it's recomputed on demand from the
 * payment ledger that already lives on the device.
 *
...
export interface BehaviorProfile {
  ...
  /** Distinct local hours-of-day (0–23) the customer has transacted in. */
  activeHours: number[];
...
/** The profile of a customer with no payment history. */
