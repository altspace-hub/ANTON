 ...
 */
export async function maybeRunIdlePoll(): Promise<number | null> {
  if (!(await isIdlePollDue())) return null;
  // Touch the chosen hour so it's persisted before the poll body runs
  // — keeps the state consistent if the user kills the app mid-poll.
  await getOrPickHour();
...
/** Force a refresh regardless of gap. Used by pull-to-refresh and the
 *  "Sync now" one-shot fallback when active-sync isn't appropriate. */
export async function runOneShotPoll(): Promise<number> {
  const fresh = await pollIncomingOnce();
  await setSecure(LAST_RUN_KEY, String(Date.now()));
  return fresh.length;
}
