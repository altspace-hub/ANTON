/**
 * money-profile.ts — the customer's self-declared money profile.
 *
 * A personal, on-device baseline: who the customer is and what their
...
const KEY = 'fc.pay.money-profile';

/** Where the customer's money mainly comes from. */
...
export interface MoneyProfile {
  ...
  occupation: string;
  // ── Money & spending ──────────────────────────────────────
  /** Where the customer's funds mainly come from. '' if not declared. */
...
/** True once the customer has declared at least one money-pattern
