/**
 * CopyRow — a labelled, monospaced value with a one-tap copy button and a
 * transient "Copied" flash. Used for long opaque values (txId, addresses, refs).
 *
 * Copy path: navigator.clipboard.writeText, falling back to selecting the value
 * text on older/insecure WebViews. Presentation-only — never mutates app state.
 *
 * Ported verbatim from src/pay/components/CopyRow.tsx (#79 wallet parity).
 */
...
export default function CopyRow({ label, value }: Props) {
  ...
  async function copy() {
    try {
      ...
      // Clipboard unavailable (insecure context / older WebView) — select the
      // value text so the user can long-press → Copy.
      try {
