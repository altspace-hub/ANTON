 ...
 * Why this exists
 * ---------------
 * The app runs in a Capacitor WebView whose origin is `https://localhost`.
 * Calls to the FutureChain hub (`https://rpc.futurechain.eu`) are therefore
 * CROSS-ORIGIN, and the hub's CORS layer rejects the `https://localhost`
 * origin with a 403 ("CORS request forbidden: origin not allowed"). The
 * browser then surfaces a bare `TypeError: Failed to fetch`, so on-device
 * the wallet could never read its own balance / UTXOs / receive history.
 *
 ...
 *
 * Kept per-app (mirrors src/pay/services/native-http.ts) so each wallet
 * bundle stays self-contained — there is no shared module across the
 * pay / comm / business builds.
 */
