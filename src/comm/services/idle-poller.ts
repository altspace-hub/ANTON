...
import { pollIncomingOnce, type FreshIncoming } from './received';
 ...
 */
export async function maybeRunIdlePoll(): Promise<FreshIncoming[] | null> {
 ...
 *  Returns the fresh inbound rows in arrival order. */
export async function runOneShotPoll(): Promise<FreshIncoming[]> {
  const fresh = await pollIncomingOnce();
  await setSecure(LAST_RUN_KEY, String(Date.now()));
  return fresh;
}
