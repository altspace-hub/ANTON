/**
 * payment-identity.ts — the Comm user's ISO 20022 debtor party.
 *
 * Every PACS.008 the Comm wallet originates names the user as the
 * *debtor*. The scanned `futurechain:pay` QR carries the creditor
 * (merchant) party; this is the other half, saved once by the user
 * in Settings → Payment details.
 *
 * The wallet address is NOT stored here — it lives in the wallet
 * service (services/wallet.ts) and is read in when a PACS.008 draft
 * is assembled.
 *
...
const KEY = 'fc.comm.payment-identity';

/** The Comm user as a PACS.008 debtor party — minus the wallet
 *  address, which is sourced from the wallet service at
 *  draft-assembly time. */
...
/** True once the user has saved a name — enough to populate the
