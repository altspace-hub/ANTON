/**
 * fraud-engine.test.ts — coverage for the deterministic light fraud
 * engine: each signal rule, plus score + level derivation.
 *
 * Kept in sync with src/pay/services/__tests__/fraud-engine.test.ts so
 * the two apps score behaviour the same way. Import paths point at the
 * Comm copies of the engine + profile types.
 */
import { describe, expect, it } from 'vitest';
import { assessPayment, type PendingPayment } from '../services/fraud-engine';
import type { MoneyProfile } from '../services/money-profile';
...
function behavior(over: Partial<BehaviorProfile> = {}): BehaviorProfile {
  return {
    count: 10,
    ...
    count24h: 1,
    total24hMicroFtc: 5_000_000n,
    total30dMicroFtc: 50_000_000n,
    ...over,
  };
}

function money(over: Partial<MoneyProfile> = {}): MoneyProfile {
  return {
...
describe('assessPayment — amount anomaly', () => {
  it('flags caution at ~3× the median', () => {
    const a = assessPayment(pending({ amountMicroFtc: 15_000_000n }), money(), behavior());
    const s = a.signals.find((x) => x.id === 'amount-anomaly');
  ...
  it('does not fire without enough history', () => {
    const a = assessPayment(
      pending({ amountMicroFtc: 25_000_000n }),
      money(),
      behavior({ count: 2 }),
    );
...
describe('assessPayment — above declared typical', () => {
  it('flags a payment far above the declared typical size', () => {
    const a = assessPayment(
      pending({ amountMicroFtc: 30_000_000n }),
      money({ typicalPaymentFtc: 5 }),
      behavior({ count: 0, medianMicroFtc: 0n }),
    );
...
describe('assessPayment — new counterparty', () => {
  it('info-flags a first payment to an unknown merchant', () => {
    ...
    const s = a.signals.find((x) => x.id === 'new-counterparty');
    expect(s?.severity).toBe('info');
    expect(a.level).toBe('clear'); // info alone does not raise the alarm
  });

  it('warns on a large payment to an unknown merchant', () => {
    const a = assessPayment(
      pending({ counterparty: 'NEWMERCH', amountMicroFtc: 12_000_000n }),
      money(),
      behavior(),
    );
  ...
  it('does not flag the very first payment ever', () => {
    const a = assessPayment(
      pending({ counterparty: 'NEWMERCH' }),
      money(),
      behavior({ count: 0, knownCounterparties: [] }),
    );
...
describe('assessPayment — velocity', () => {
  it('cautions at 4 payments in 24h', () => {
    const a = assessPayment(pending(), money(), behavior({ count24h: 4 }));
    expect(a.signals.find((x) => x.id === 'velocity')?.severity).toBe('caution');
  });

  it('warns at 8 payments in 24h', () => {
    const a = assessPayment(pending(), money(), behavior({ count24h: 8 }));
    expect(a.signals.find((x) => x.id === 'velocity')?.severity).toBe('warning');
  });
});
...
describe('assessPayment — monthly cap', () => {
  it('cautions when the payment pushes past the declared monthly figure', () => {
    const a = assessPayment(
      pending({ amountMicroFtc: 5_000_000n }),
      money({ expectedMonthlyFtc: 52 }),
      behavior({ total30dMicroFtc: 50_000_000n }),
    );
  ...
  it('warns when far past the declared monthly figure', () => {
    const a = assessPayment(
      pending({ amountMicroFtc: 40_000_000n }),
      money({ expectedMonthlyFtc: 52 }),
      behavior({ total30dMicroFtc: 50_000_000n }),
    );
...
describe('assessPayment — odd hour', () => {
  it('cautions when the payment hour is outside the usual active hours', () => {
    const a = assessPayment(
      pending(),
      money(),
      behavior({ activeHours: [(NOW_HOUR + 3) % 24, (NOW_HOUR + 4) % 24] }),
    );
  ...
  it('does not fire when the hour is a usual one', () => {
    const a = assessPayment(pending(), money(), behavior());
    expect(a.signals.find((x) => x.id === 'odd-hour')).toBeUndefined();
  });
});
...
describe('assessPayment — expiring QR', () => {
  it('cautions when the QR expires within a minute', () => {
    const a = assessPayment(
      pending({ expUnixSeconds: Math.floor(NOW / 1000) + 30 }),
      money(), behavior(),
    );
  ...
  it('does not fire for a QR with plenty of time left', () => {
    const a = assessPayment(
      pending({ expUnixSeconds: Math.floor(NOW / 1000) + 600 }),
      money(), behavior(),
    );
...
describe('assessPayment — dormancy break', () => {
  it('cautions on a large payment after a long quiet gap', () => {
    const a = assessPayment(
      pending({ amountMicroFtc: 12_000_000n }),
      money(),
      behavior({ lastPaymentAt: NOW - 90 * DAY }),
    );
