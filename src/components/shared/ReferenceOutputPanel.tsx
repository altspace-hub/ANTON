  ...
  return (
    <div>
      ...
      >
        ...
        {value.trim().length > 0 && (
          <span className="rounded-full bg-adv-teal/20 px-1.5 py-0.5 text-[10px] text-adv-teal">set</span>
        )}
      </button>

      {expanded && (
        <div className="mt-2 rounded-lg border border-border bg-adv-dark-2 p-3">
          <p className="mb-2 text-[11px] text-adv-gray-med">
          ...
          <div className="relative">
            <textarea
              ...
              className="w-full rounded-lg border border-border bg-adv-dark p-2.5 pr-8 text-xs text-adv-off-white placeholder:text-adv-gray-med focus:border-adv-teal focus:outline-none focus:ring-1 focus:ring-adv-teal"
              rows={6}
            />
            {value.trim().length > 0 && (
              <button
                onClick={() => onChange('')}
                className="absolute right-2 top-2 text-adv-gray-med hover:text-adv-red transition-colors"
