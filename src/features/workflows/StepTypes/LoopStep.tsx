  ...
  return (
    <div className="space-y-3">
      <div className="rounded-lg border border-adv-gold/30 bg-adv-gold/10 p-3">
        <p className="text-[10px] font-medium text-adv-gold">Loop Step</p>
      ...
      <div>
        <label className="mb-1 block text-[11px] font-medium text-adv-gray">
          Input List Path
          <span className="ml-1 text-adv-gray-med font-normal">Step output path containing an array</span>
        </label>
        <input
          type="text"
      ...
      <div>
        ...
        {loopSteps.length === 0 && (
          <div className="rounded-lg border border-dashed border-border py-4 text-center">
            <p className="text-[11px] text-adv-gray-med">No loop steps defined. Add a step to execute per item.</p>
          </div>
        )}
        ...
        <div className="space-y-2">
          {loopSteps.map((ls, idx) => (
            <div key={ls.id} className="flex items-center gap-2 rounded-lg border border-border bg-adv-dark p-2">
              <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded bg-adv-teal/10 text-[10px] font-bold text-adv-teal">
                {idx + 1}
              </div>
              <input
                type="text"
                ...
                className="flex-1 rounded border border-border bg-adv-dark-2 px-2 py-1 text-[11px] text-adv-off-white focus:border-adv-teal focus:outline-none"
              />
              <select
                value={ls.type}
              ...
              <button onClick={() => removeLoopStep(idx)} className="text-adv-gray-med hover:text-adv-red transition-colors">
