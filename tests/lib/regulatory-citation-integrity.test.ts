...
function loadFramework(
  ...
  // highest article it lists, and the last article of its last division. MiCA's
  // file declares 39 because it only carries 39 of the 149 articles.
  ...
  // from the file proves absence from the regulation. True for AMLR (90 of 90),
  // false for MiCA (39 of 149).
...
describe('AMLR / MiCA citations match the articles that carry the obligation', () => {
  it('the framework files the guard stands on are the ones it thinks they are', () => {
    ...
    // MiCA is a SPARSE selection (39 of 149): absence proves nothing there, so
    // the guard only uses its division spans and its article ceiling.
    expect(MICA.complete).toBe(false);
