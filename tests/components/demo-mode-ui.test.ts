...
describe('DemoBanner', () => {
  it('warns a visitor, states the retention period and links the privacy notice', async () => {
    ...
    expect(container.textContent).toContain('answers are generated by AI (GLM 5.3 Flash, via OpenRouter, USA) and can be wrong.');
    expect(container.textContent).toContain("Don't enter personal or client data.");
...
describe('LoginPage on a demo', () => {
  ...
  async function fillAndSubmit(code = 'the-code') {
    ...
    for (const id of ['#su-over18', '#su-accept-terms']) {
  ...
  it('offers no sign-up on an ordinary team server (negative control)', async () => {
    ...
    expect(container.textContent).not.toContain("Don't enter personal");
  });
});
...
describe('PrivacyNoticePage', () => {
  it('is marked DRAFT, has placeholders for the controller, and states the retention period', async () => {
    ...
    expect(container.textContent).toContain('DRAFT: this privacy notice is not yet in force.');
    expect(container.textContent).toContain('[[CONTROLLER_NAME]]');
    expect(container.textContent).toContain('21 days');
