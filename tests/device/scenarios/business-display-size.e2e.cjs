/**
 * business-display-size.e2e.cjs — Business screen-size adaptation on-device.
 *
 * Verifies the display-size CSS MECHANISM applies in Business's shell (height:100% +
 ...
 *
 * Requires ANTON_BUSINESS_SERIAL (or the default device) = the funded phone.
 */
...
module.exports = {
  name: 'business-display-size',
  apps: ['business'],
  async run({ log }) {
    const business = await forwardApp('business', 0);
    const s = new CdpSession(business.wsUrl); await install(s);
    ...
    try {
      ...
      log(`Business display CSS: base=${base.fontPx} large=${large.fontPx} compact=${compact.fontPx} tablet=${tablet.fontPx}/${tablet.appMaxWidth}`);
      ...
      // Picker presence is best-effort (Business's settings nav varies); log either way.
      log(`Business settings picker visible: ${ui.hasPicker}`);
    } finally {
      s.close();
    }
  },
