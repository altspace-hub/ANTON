/**
 * business-overlay-column.e2e.cjs — verify the fixed-overlay column constraint.
 *
 ...
 * Requires ANTON_BUSINESS_SERIAL = the funded phone.
 */
...
module.exports = {
  name: 'business-overlay-column',
  apps: ['business'],
  async run({ log }) {
    const a = await forwardApp('business', 0);
