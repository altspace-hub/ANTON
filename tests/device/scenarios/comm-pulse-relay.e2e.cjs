/**
 * comm-pulse-relay.e2e.cjs — TWO-PHONE: a Pulse post fans out over the relay
 * to a peer's feed. Alice composes a post (audience = Everyone, the default);
 * we poll Bob's `pulse_posts` store until the post arrives + applyInboundPost
 * stores it.
 *
 * SLOW BY DESIGN: publishPulsePost jitters each recipient send across a 0–30 s
 ...
 * fanout that the single-phone comm-pulse scenario can't.
 *
...
module.exports = {
  name: 'comm-pulse-relay',
  apps: ['comm'],
  async run({ log }) {
    const marker = 'pulse-relay-' + Date.now();
    ...
    try {
      // Alice composes + publishes a Pulse post (audience Everyone default).
      const posted = await as.eval(`(async () => {
        ...
        const mine = (await __td.readStore('anton-comm', 'pulse_posts')).some((p) => (p.text || '').includes(${JSON.stringify(marker)}));
        return { ok: mine };
      })()`);
      ...
      for (let i = 0; i < 27 && !got; i++) {
        const rows = await bs.eval("__td.readStore('anton-comm', 'pulse_posts')");
        got = (rows || []).find((p) => (p.text || '').includes(marker) && p.authorHash !== undefined);
        if (!got) await new Promise((r) => setTimeout(r, 1500));
      }
      assert.ok(got, "Bob's feed received Alice's Pulse post via relay fanout (within ~40s)");
