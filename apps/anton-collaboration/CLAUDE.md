...
- **The three committing AGREE verbs fail closed under `--mcp-stdio`.** `proposeAgreement` / `acceptAgreement` / `counterAgreement` need a human-approval driver, which the MCP transport does not have (stdin belongs to MCP) — so they are **omitted from the MCP tool list and fail closed with `-32011`**. Everything else works over MCP: the **task inbox**, discovery, talk, negotiate, settlement-instruction, fulfilment, and escrow records. You can run the whole "pick up a task → discover → negotiate → settle via the payment gateway → report back" loop over MCP, gating the actual spend in the payment gateway. To *commit* an agreement, drive the JSON-RPC transport (terminal approval) instead.
