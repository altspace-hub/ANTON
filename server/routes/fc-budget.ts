...
// Admin-only in team mode: the spending rules and log are the instance
// wallet's, one per instance, not per user.
...
export async function createFCBudgetRoutes(db: DatabaseAdapter): Promise<Router> {
  ...
  router.get('/futurechain/budget/rules', requireAdminOrSolo, async (_req, res) => {
    try {
  ...
  router.put('/futurechain/budget/rules', requireAdminOrSolo, async (req, res) => {
    try {
  ...
  router.get('/futurechain/budget/state', requireAdminOrSolo, async (_req, res) => {
    try {
  ...
  router.get('/futurechain/budget/log', requireAdminOrSolo, async (req, res) => {
    try {
  ...
  router.post('/futurechain/budget/check', requireAdminOrSolo, async (req, res) => {
    try {
