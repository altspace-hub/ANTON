...
export async function createFinanceRoutes(db: DatabaseAdapter, anthropic?: Anthropic) {
  const router = Router();
  ...
  for (const sql of financeTables) {
    try { await db.exec(sql); } catch (e) { console.warn('[finance] table migration warning:', e); }
  }
  ...
  router.post('/finance/watchlist', async (req, res) => {
    try {
  ...
  router.delete('/finance/watchlist/:id', async (req, res) => {
    try {
  ...
  router.post('/finance/goals', async (req, res) => {
    try {
      ...
      await db.run(`INSERT INTO finance_goals (id, user_id, goal_type, title, target_amount, current_amount, currency, target_date, monthly_contribution, parameters) VALUES (?,?,?,?,?,?,?,?,?,?)`
      , 
        id, 'default',
  ...
  router.patch('/finance/goals/:id', async (req, res) => {
    try {
      const body = req.body as Record<string, unknown>;
      const fields: string[] = [];
  ...
  router.get('/finance/learning-progress', async (req, res) => {
    try {
  ...
  router.post('/finance/learning-progress', async (req, res) => {
    try {
...
- Maximum 250 words`,
  ...
  router.post('/finance/calculate', async (req, res) => {
    try {
