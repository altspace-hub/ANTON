...
export async function composeSystemPrompt(config: PromptComposerConfig): Promise<string> {
  ...
  if (typeof config.projectContextPrompt === 'string' && config.projectContextPrompt.trim()) parts.push(config.projectContextPrompt.trim());
...
export async function composeSystemPromptSplit(config: PromptComposerConfig): Promise<ComposedSystemPrompt> {
  ...
  // Layer 4b: Project context — the matter's prior conclusions
  if (typeof config.projectContextPrompt === 'string' && config.projectContextPrompt.trim()) staticParts.push(config.projectContextPrompt.trim());
