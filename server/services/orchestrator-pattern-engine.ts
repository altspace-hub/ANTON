...
function detectQualityDropPatterns(db: Database.Database): DetectedPattern[] {
  ...
  try {
    const rows = db.prepare(`
      ...
      HAVING recent_avg IS NOT NULL AND prior_avg IS NOT NULL
        AND prior_avg - recent_avg >= 1.5
        AND total_scores >= 4
      ORDER BY (prior_avg - recent_avg) DESC
      LIMIT 5
    `).all() as Array<{ module_id: string; recent_avg: number; prior_avg: number; total_scores: number }>;
...
function detectWorkflowRecurrencePatterns(db: Database.Database): DetectedPattern[] {
  const patterns: DetectedPattern[] = [];
  try {
    const rows = db.prepare(`
      ...
      LIMIT 8
    `).all() as Array<{ workflow_id: string; run_count: number; first_run: string; last_run: string; success_rate: number }>;
...
function detectSignalClusterPatterns(db: Database.Database): DetectedPattern[] {
  const patterns: DetectedPattern[] = [];
  try {
    ...
    const radarClusters = db.prepare(`
      ...
      GROUP BY item_type
      HAVING count >= 3
    `).all() as Array<{ item_type: string; count: number; avg_urgency: number }>;
...
function detectDeadlineClusterPatterns(db: Database.Database): DetectedPattern[] {
  const patterns: DetectedPattern[] = [];
  try {
    ...
    `).get() as { count: number; titles: string | null; earliest: string | null } | undefined;
 ...
 */
export function shouldAutoPause(db: Database.Database): { pause: boolean; reason: string } {
  try {
    ...
    `).get() as { bad: number; total: number } | undefined;
 ...
 */
export function detectPatterns(db: Database.Database): DetectedPattern[] {
  ...
  try {
    ...
    ).all() as Array<{ signal_data: string | null }>;
 ...
 */
export function recordPatternDetection(
  ...
  try {
    ...
    const existing = db.prepare('SELECT id FROM orchestrator_patterns WHERE id = ?').get(pattern.pattern_id);
    if (!existing) {
      db.prepare(`
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, 0)
    `).run(detectionId, pattern.pattern_id, JSON.stringify({ ...pattern.signal_data, pattern_id: pattern.pattern_id }), proposalId);
...
export function getAutoExecutionCount(db: Database.Database): number {
  try {
    const r = db.prepare(`
      SELECT COUNT(*) as c FROM orchestrator_pattern_detections
      WHERE detected_at >= datetime('now', 'start of day')
        AND auto_executed = 1
    `).get() as { c: number };
    return r.c;
  } catch { return 0; }
...
export function isAutoExecutionAllowed(db: Database.Database): boolean {
