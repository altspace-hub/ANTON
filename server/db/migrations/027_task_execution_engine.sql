...
ALTER TABLE anton_tasks ADD COLUMN intake_answers TEXT DEFAULT '{}';
-- JSON object: gathered key-value context from intake Q&A (entity, jurisdiction, etc.)

ALTER TABLE anton_tasks ADD COLUMN execution_results TEXT DEFAULT '[]';
-- JSON array of {step, name, output, at} objects — one per completed execution step

ALTER TABLE anton_tasks ADD COLUMN current_step INTEGER DEFAULT 0;
-- Index of the next step to execute (0-based into approach.execution_steps)

ALTER TABLE anton_tasks ADD COLUMN intake_ready INTEGER DEFAULT 0;
