...
export function initDatabase(): Database.Database {
  ...
  if (complianceRulesExist.c === 0) {
    db.prepare(`INSERT INTO compliance_rules (rule_code, title, description, category, severity, regulatory_source, rule_logic, active, auto_remediate) VALUES
      ...
      ('MODEL_WHITELIST_001', 'Approved Models Only', 'Only allow use of approved Claude models for compliance work', 'governance', 'critical', 'Data Governance Policy', '{"type":"lookup","config":{"field":"model","allowedValues":["claude-opus-4-8","claude-opus-4-7","claude-opus-4-6","claude-sonnet-4-6","claude-sonnet-4-5-20250929"]}}', 1, 0),
  ...
  createKnowledgePackService(new SqliteAdapter(db))
    .then((kpService) => {
      ...
      for (const dir of packDirs) {
        const dirPath = path.join(packsDir, dir);
