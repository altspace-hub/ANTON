...
| 2026-04-18 | 10.1 — Hardening pass from 5-expert double-take review (8 critical · 9 improvements · 6 polish) | **Critical (8):** workspace path-traversal blocked in `quality-adapters/_shared.ts` (resolves explicit `metadata.workspace_path` against `HARDWARE_WORKSPACES_ROOT`, falls back to default on escape) · `hkp-service.listPacks` N+1 collapsed from `1+N×5` to `1+5` (parallel grouped `WHERE hkp_id IN (...)` queries merged via `Map`) · race-safe `maintain.advanceStage` with `UPDATE … WHERE id=? AND status=? RETURNING *` so concurrent calls can't both win · idempotent `diagnose.logOutcome` via partial-UNIQUE + `ON CONFLICT … DO UPDATE` + `xmax=0` first-insert detection (no double counter bumps) · 7 `parseJson<T>` copies + `sha256()` deduplicated into `server/lib/hardware-helpers.ts` · `bundleHumanitarianDeploymentKit` HKP-fetch logic extracted to shared `loadHkpData()` helper, both bundlers reuse it · 3 native `confirm()` calls replaced with accessible `ConfirmModal` (severity, typed-confirm, reason ≥10 chars, ARIA, Esc + Cmd+Enter) · `grid-cols-1 lg:grid-cols-3` → `grid-cols-1 md:grid-cols-2 lg:grid-cols-3` across 6 hardware pages (proper tablet breakpoint). **Improvements (9):** `ServiceError` class with HTTP status + `statusFromError()` mapper applied to all 77 catches in `routes/hardware.ts` · lazy `regulatory-pack-service` import in `hardware-project-service` (breaks startup-time circular dependency risk) · `checkSchemaVersion()` JSONB schema-version forward-compat hook · regulatory page acronym glossary (CRA, RED, MDR, DoC, VDP, DPA) inside collapsible `<details>` · `console.warn` traversal-attempt logging flagged for telemetry pipe in Phase 11 · `src/theme/hardware-status.ts` centralises `PATH_BADGES` / `TIER_BADGES` / `VERDICT_STYLES` / `OUTCOME_STYLES` / `RISK_STYLES` / `severityClass()` / `relativeTime()`. **Polish (6):** Migration `144_hardware_hardening.sql` adds index on `hw_patch_stages.status` + partial UNIQUE on `diagnostic_case_outcomes(case_id, resolution_id, contributor_id) WHERE contributor_id IS NOT NULL` + deferrable soft FK on `hardware_projects.current_phase_id` + NOTIFY trigger on `hw_community_review_queue` for real-time reviewer dashboards · `SkeletonCard` shimmer placeholder for templates list · `relativeTime()` "2h ago" rendering on diagnose reasoning trail with full timestamp on hover · empty-state on `HardwareBuildPage` now offers secondary "Browse templates →" CTA. | Hardening smoke (then deleted): workspace-traversal 3/3 escape attempts blocked + default-path fallback · `listPacks` 56ms with no N+1 · idempotent outcome `r1.id === r2.id` · concurrent `advanceStage` race: 1 fulfilled + 1 rejected (correct enforcement). TypeScript clean. **Honest scope on Phase 10.1:** This closes the gap between "code-complete" and "production-ready before external pilot." All 23 review items implemented; not yet pilot-tested with real users (that remains Phase 10's manual go/no-go in `HARDWARE_BUILD_LAUNCH_CHECKLIST.md`). |
