...
|---|---|---|---|
| Anthropic | `ANTHROPIC_API_KEY` | `claude-opus-4-6` | Built-in (`server/services/claude-client.ts`) |
...
For Opus 4.6: use `thinking: { type: 'adaptive' }` with `output_config: { effort }` as a **separate** top-level parameter. Never put `effort` inside `thinking`. Never set `budget_tokens` for Opus.
