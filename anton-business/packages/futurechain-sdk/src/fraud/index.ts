/**
 * @futurechain/sdk/fraud — the light fraud engine.
 *
 * A pure, deterministic scorer (no I/O, no LLM, no network) — the same
 * "deterministic engine" shape as the Risk Atlas. It compares a pending payment
 * against two baselines (the self-declared money profile + the derived behaviour
 * profile) and returns a risk level + a list of plain-language signals.
 *
 * ADVISORY only. The UI shows the signals before the customer confirms; it never
 * blocks a payment. The goal is to help a person spot a scam or a mistake — "is
 * this normal *for me*?" — not to run a regulated AML system (Heimdall screens
 * sanctions at the node). Every signal carries an i18n `messageKey`, so the
 * engine stays language-agnostic and fully testable.
 *
 * 2026-07-17: promoted here from the byte-identical per-app copies in ANTON Pay
 * and Comm (src/{pay,comm}/services/fraud-engine.ts). The engine is the shared
 * ALGORITHM; each app keeps building its own behaviour/money profiles (which
 * legitimately differ). The inputs are STRUCTURAL — an app's fuller profile
 * types satisfy these read-only shapes by duck typing.
 */
...
/** The behaviour-baseline fields the scorer reads. An app's fuller
 *  BehaviorProfile structurally satisfies this. */
export interface FraudBehaviorProfile {
  count: number;
...
/** The money-profile fields the scorer reads. */
export interface FraudMoneyProfile {
...
/** Has the customer transacted with this counterparty before? */
export function isKnownCounterparty(
  profile: Pick<FraudBehaviorProfile, 'knownCounterparties'>,
  counterparty: string,
): boolean {
...
/**
 * Assess a pending payment. Pure + deterministic — same inputs always yield the
 * same assessment.
 */
export function assessPayment(
  pending: PendingPayment,
  money: FraudMoneyProfile | null,
  behavior: FraudBehaviorProfile,
): FraudAssessment {
